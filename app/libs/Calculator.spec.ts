...
describe('Тестовий набір для Calculator.ts (повне QA-покриття з stress-тестами)', () => {
  ...
  it('stress-тест комбінації %, +/-, крапки і великих чисел', () => {
    calculator.dashboard.value = '';
    const sequence = ['1', '2', '3', '.', '4', '5', '+', '-', '*', '/', '%', '+/-'];
    for (let i = 0; i < 50; i++) {
      for (const val of sequence) {
        if (val === '+/-') calculator.printAction(val);
        else if (['+', '-', '*', '/', '.', '%'].includes(val)) calculator.printAction(val);
        else calculator.printDigit(val);
      }
    }
    expect(calculator.dashboard.value.length).toBeGreaterThan(0);
    expect(() => calculator.solve()).not.toThrow();
  });
  // ---------------- Ініціалізація ----------------
