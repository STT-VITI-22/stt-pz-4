...
describe('Тестовий набір для Calculator.ts (повне QA-покриття з stress-тестами)', () => {
  ...
  it('введення дуже довгого рядка (1000 цифр) не падає', () => {
    ...
    // ---------------- Функція очищення (clr) ----------------
  it('clr() очищує dashboard до порожньої строки', () => {
    // Спочатку заповнюємо dashboard якимось значенням
    calculator.dashboard.value = '123 + 456 * 789';
    calculator.clr();
    
    expect(calculator.dashboard.value).toBe('');
  });

  it('clr() працює коли dashboard вже порожній', () => {
    // Переконуємось, що dashboard порожній
    calculator.dashboard.value = '';
    calculator.clr();
  ...
  it('clr() очищує dashboard після обчислення', () => {
    const expr = '2+2*2';
    calculator.dashboard.value = expr;
    calculator.solve(); // припускаємо, що solve() обчислює і записує результат назад у dashboard
    
    calculator.clr();
    
    expect(calculator.dashboard.value).toBe('');
    // Додатково перевіряємо, що після очищення обчислення не падає
